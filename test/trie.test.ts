...
import { unstable_create_router as createRouter } from "../src/unstable_router.js"
...
describe("createRouter", () => {
    ...
    describe("root", () => {
        test("Accessing root path", async () => {
            const rootEndpoint = createEndpoint("GET", "/", async () => {
                return Response.json({ message: "Root path" }, { status: 200 })
            })

            const router = createRouter([rootEndpoint])
            const { GET } = router

            const get = await GET(new Request("https://example.com/", { method: "GET" }))
            expect(get.status).toBe(200)
            expect(get.ok).toBeTruthy()
            expect(await get.json()).toEqual({ message: "Root path" })
        })
    })
})
