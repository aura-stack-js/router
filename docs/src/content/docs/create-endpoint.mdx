...
`request` is the first argument passed to every route handler. It contains the original, unmodified `Request` object from the incoming HTTP request.
...
const signIn = createEndpoint("/GET", "/auth/signIn/:oauth", async (request) => {
...
###### Search Parameters Context (`ctx.searchParams`) [#search-params-context]
